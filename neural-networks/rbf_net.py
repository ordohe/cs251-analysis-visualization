...
class RBF_Net:
    ...
    def avg_cluster_dist(self, data, centroids, cluster_assignments, kmeans_obj):
        ...
        for i in range(self.k):
            ...
            dists = kmeans_obj.dist_pt_to_centroids(centroids[i], points)
    ...
    def initialize(self, data):
        ...
        '''
        kmeans = KMeans(data)
        kmeans.cluster(k = self.k)
        self.prototypes = kmeans.get_centroids()
        clusters = kmeans.get_data_centroid_labels()
        avg_dists = self.avg_cluster_dist(data, self.prototypes, clusters, kmeans)
        self.sigmas = avg_dists
    ...
    def linear_regression(self, A, y):
        ...
        '''
        int = np.hstack((A, np.ones((A.shape[0], 1))))
        c, _, _, _ = np.linalg.lstsq(int, y, rcond = None)
        return c
    ...
    def hidden_act(self, data):
        ...
        for i in range(self.k):
            for j in range(data.shape[0]):
                distance_sq = np.sum((data[j] - self.prototypes[i])**2)
                activations[j, i] = np.exp(-distance_sq / (2 * self.sigmas[i]**2 + 1e-8))
        return activations
    ...
    def output_act(self, hidden_acts):
        ...
        output_activations = np.dot(hidden_acts_with_bias, self.wts)
    ...
    def train(self, data, y):
        ...
        for i, cls in enumerate(classes):
            encoded_y[:, i] = (y == cls).astype(int)
        self.wts = self.linear_regression(hidden_acts, encoded_y)
