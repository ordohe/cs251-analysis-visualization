...
class CatData(data.Data):
    ...
    def get_cat_levels_int(self, header):
        ...
        levels = np.unique(self.data[:, col_index])
        int_levels = np.arange(len(levels))
    ...
    def int2strlevels(self, header, int_levels):
        ...
        return [levels[i] for i in int_levels]
    ...
    def str2intlevels(self, header, str_levels):
        ...
        '''
        levels = self.cats2levels[header]
        return [levels.index(level) for level in str_levels]
    ...
    def filter(self, header, strlevel):
        ...
        '''
        col_index = self.header2col[header]
        levels = self.get_cat_levels_str(header)
        int_level = levels.tolist().index(strlevel)
        filtered_data = self.data[self.data[:, col_index] == int_level]
